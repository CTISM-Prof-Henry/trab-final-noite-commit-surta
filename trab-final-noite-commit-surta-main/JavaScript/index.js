...
function consultarCadastros() {
  // Abre o modal que lista os cadastros
  ...
  // Substitui os listeners antigos para evitar duplicação
  if (filtroBlocoEl) filtroBlocoEl.onchange = atualizarListaCadastros;
  if (filtroTipoEl) filtroTipoEl.onchange = atualizarListaCadastros;
  if (filtroCapEl) filtroCapEl.oninput = atualizarListaCadastros;

  // Atualiza lista de cards imediatamente
  atualizarListaCadastros();
}

function atualizarListaCadastros() {
  const modalBody = document.querySelector('#modaltres .modal-body');
  if (!modalBody) return console.error('Modal #modaltres ou .modal-body não encontrado');
  ...
  // Remove conteúdo antigo (exceto filtros)
  Array.from(modalBody.children).forEach(child => {
    if (child !== filtrosDiv) child.remove();
  });

  // Cria ou seleciona container para cards
  ...
  if (!Array.isArray(salasCadastradas) || salasCadastradas.length === 0) {
    cardsRow.innerHTML = `
      <div class="col-12">
  ...
  // Se nenhuma sala corresponde aos filtros
...
if (sala === 'Auditório') {
  ...
  salasCadastradas.push({
    ...
    sala,
    capacidade,
   tipo: sala === 'Auditório' ? 'Auditório' : tipo,
...
const diaSemana = getDiaSemana(dataLocal);
  ...
  if (sala === 'Auditório' && capacidade > 100) {
...
const hIni = parseInt(horaInicial.replace(":", ""));
const hFim = parseInt(horaFinal.replace(":", ""));
...
// Validação: horários permitidos (07:30 até 23:00)
const limiteInicio = 7 * 60 + 30;   // 07:30
const limiteFim = 23 * 60;          // 23:00

const [hiHora, hiMin] = horaInicial.split(':').map(Number);
const [hfHora, hfMin] = horaFinal.split(':').map(Number);

const totalIni = hiHora * 60 + hiMin;
const totalFim = hfHora * 60 + hfMin;

// Antes das 07:30
if (totalIni < limiteInicio) {
  alert('O horário INICIAL não pode ser antes de 07:30.');
  return;
}

// Depois das 23:00
if (totalFim > limiteFim) {
  alert('O horário FINAL não pode ser depois de 23:00.');
  return;
}
...
function consultarAgendamentos() {
  // Mostra modal de consulta
  $('#modaldois').modal('show');

  // Adiciona os event listeners **apenas depois que a modal estiver aberta**
  $('#modaldois').on('shown.bs.modal', function () {
    const filtroResponsavel = document.getElementById('filtroResponsavel');
    const filtroBloco = document.getElementById('filtroBlocoAgendamento');
    const filtroPeriodo = document.getElementById('filtroPeriodoAgendamento');

    if (filtroResponsavel) filtroResponsavel.addEventListener('input', atualizarCardAgendamentos);
    if (filtroBloco) filtroBloco.addEventListener('change', atualizarCardAgendamentos);
    if (filtroPeriodo) filtroPeriodo.addEventListener('change', atualizarCardAgendamentos);
